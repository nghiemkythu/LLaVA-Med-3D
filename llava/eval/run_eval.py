...
def evaluate(gt, pred, candidate, criterion=None):    
    ...
    for gt_item, pred_item in zip(gt, pred):
        ...
        if gt_item['answer_type'] == 'OPEN':
            is_correct = gt_value in pred_value
            open_correct += is_correct
            open_hit_scores['hit'].append(is_correct)
            ...
            closed_scores['hit'].append(is_correct)
    ...
    with open('iclr_rebuttal/evaluation_stats.csv', 'w', newline='') as csvfile:
        fieldnames = ['id', 'type', 'correct', 'recall']
    ...
    print(f'open_correct {open_correct} || closed_correct {closed_correct}')
    return tabulate(
        [
...
#    assert gt_ids == pred_ids, "please make sure pred and gt are exactly matched"
    ...
    with open('iclr_rebuttal/evaluation_results.txt', 'w') as file:
